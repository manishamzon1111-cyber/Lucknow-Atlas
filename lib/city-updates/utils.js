...
export function processItems(rawItems) {
  ...
  items.sort((a, b) => {
    const eventA =
      a.eventStart
        ? Date.parse(a.eventStart)
        : NaN;

    const eventB =
      b.eventStart
        ? Date.parse(b.eventStart)
        : NaN;

    const upcomingA =
      Number.isFinite(eventA) &&
      eventA >= now - 86400000;

    const upcomingB =
      Number.isFinite(eventB) &&
      eventB >= now - 86400000;
  ...
  for (const item of items) {
    ...
    /*
     * Enough variety without letting one source
     * or one category take over the drawer.
     */
    if (
      counts[item.type] >= 3 ||
