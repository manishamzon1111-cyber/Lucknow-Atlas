...
async function runQuery(query) {
  ...
  return parseRSS(xml)
    ...
    .filter(item => {
      /*
       * Every query is already explicitly scoped to Lucknow
       * or a named Lucknow venue.
       *
       * Requiring "Lucknow" again inside the headline was
       * discarding legitimate items such as:
       * "Heritage walk traces 150-yr-old Imambada..."
       *
       * We still require the headline itself to prove the
       * CATEGORY and we still hard-block junk.
       */
      const categoryMatch =
        containsAny(
          item.title,
